...
There are a few you should know about. HEAD points to a branch ref, which points to a commit. And in detached HEAD (we'll get to that in the next section), it points directly to a commit. So, if you have just created a branch, it points to the `main` branch. If you checkout a new branch, it points to that one.

Branches are also basically just a pointer to a commit, more specifically, the latest commit on that branch. In fact, since git handles a lot of things with just basic files, you can see which commits all your branches point to by just opening the corresponding file in `.git/ref/heads`.
...
And honestly, the best way to learn this stuff is still to just break things in a small repo and then fix them. Committed changes are very, very hard to lose when using git, atleast as long as you know `git reflog`.
